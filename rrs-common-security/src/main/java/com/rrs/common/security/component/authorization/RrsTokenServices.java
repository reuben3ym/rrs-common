 ...
 * @author Luke Taylor
 * @author Dave Syer
 * @author zjm 修改了 createAccessToken refreshAccessToken 方法 (only update createAccessToken refreshAccessToken method)
 * 修改后实现：
 * 1 每次登录都刷新了 accessToken，并且加满了过期时间，原 accessToken n 秒内仍有效
 * 2 用 refreshToken 刷新 accessToken，原 accessToken n 秒内仍有效
 */
public class RrsTokenServices implements AuthorizationServerTokenServices, ResourceServerTokenServices, ConsumerTokenServices, InitializingBean {
    ...
    public RrsTokenServices() {
    }

    public RrsTokenServices(int oldAccessTokenValiditySeconds) {
